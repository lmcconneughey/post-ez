...
const PostInteractions = ({
    ...
    const handleLike = async () => {
        ...
        setState((prev) => {
            return {
                ...prev,
                Like: prev.isLiked ? prev.Like - 1 : prev.Like + 1,
    ...
    const handleSavePost = async () => {
        setOptimisticCount('SavedPost');
        await savedPostAction(postId);
        setState((prev) => {
            return {
                ...prev,
    ...
    const [optimisticCount, setOptimisticCount] = useOptimistic(
        state,
        (prev, type: 'Like' | 'reposts' | 'SavedPost') => {
            if (type === 'Like') {
                return {
                    ...prev,
                    Like: prev.isLiked ? prev.Like - 1 : prev.Like + 1,
            ...
            return prev; // if not type, return prev
        },
    );
    ...
    return (
        <div className='flex items-center justify-between gap-8 lg:gap-16 my-2 text-textGray'>
            <div className='flex items-center justify-between gap-4 flex-1'>
                ...
                {/* repost */}
                <form action={handleReposts}>
                    <button className='group flex items-center cursor-pointer space-x-2'>
                        <svg
                            xmlns='http://www.w3.org/2000/svg'
                            width='20'
                            height='20'
                        ...
                        >
                            {Number(optimisticCount.reposts)}
                        </span>
                    </button>
                </form>
                {/* likes */}
                <form action={handleLike}>
                    <button className='group flex items-center cursor-pointer space-x-2'>
                        ...
                        >
                            {Number(optimisticCount.Like)}
                        </span>
                    </button>
                </form>
            </div>
            <form action={handleSavePost} className='flex items-center gap-2'>
